...
void setLimit()
{
	...
	int poolSize = 0;
	printf ("%s","Please enter the thread pool size: ");
	//get the whole input including the spaces
	scanf ("%d", &poolSize);
		...
		if(strcmp(token, "clr") == 0){
			...
			if(strlen(token) < 5){
				...
				if(strcmp(token, "frand") == 0){
					...
					pthread_create (&newthread, NULL, (void *)&write_to_file, &readParams);
					...
					if (fp == NULL){
						...
						pthread_create (&newthread, NULL, (void *)&sort_file, &readParams);
				    }   
				}else{
					system(token);
				}
